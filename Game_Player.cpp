...
public:
    ...
    Tank(int num0, int type0, double x0, double y0, double a0, double b0, int hp0)
    {
        ...
        cur_hp.setSize(sf::Vector2f(5 * hp, 30));
        max_hp.setSize(sf::Vector2f(500, 30));
        cur_hp.setFillColor(sf::Color::Green);
        max_hp.setFillColor(sf::Color::Red);
...
int main()
{
    ...
    while(window.isOpen())
    {
        ...
        if(tank_num == -1)
        {
            status = client.receive(buffer, 100010, bytesRead);
            if(status != sf::Socket::Done)
            {
            ...
            {
                tank_num = buffer[0];
                cout << "Tank number got: " << tank_num << " " << bytesRead << endl;
            }
        ...
        for(int i = 0; i < bytesRead; i++)
        {
            ...
            if(obj_type == 'T')
            {
                ...
                Tank(num, type, x, y, a, b, hp).draw();
            }
            else if(obj_type == 'B')
            {
                Bullet(type, x, y, a, b).draw();
            }
        }
        ...
        for(auto it = bullets.begin(); it != bullets.end(); it++)
            data += bullets[i]->data();
        string data = to_string(tanks_num);
